...
const MatchedIngredients : React.FC<Props> = ({foundMatch, setMatch}) => {
    ...
    ingredient && ingredient.toLowerCase().startsWith(value.toLowerCase()) ? true : false );
   ...
   return foundMatch && ( matchedIngredients.map((ingredient , index) => 
        <ul key={index} className="mt-3 hover:bg-[#B6D7B0]" style={{fontFamily : "var( --font-anek)", color : "var(--color-primary)"}} 
        onMouseDown={() => handleSelect(ingredient)}>{ingredient}</ul>));
}
