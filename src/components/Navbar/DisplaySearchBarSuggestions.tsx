...
import { UserValueContext, userValueContextType } from "./UserValueContext";
...
const DisplaySearchBarSuggestions : React.FC<Props> = ({ setAsMisspelled, foundMatch}) => {
    ...
    // to the value
    const findMisspelledWords = MisspelledDataArr.filter((array) => array.some((word) => 
    ...
    const suggestedWords = searchBarData.filter((item) => findMisspelledWords.some((array) => array.some((word) => word.toLowerCase().includes(item.toLowerCase()))));
    
    // Update isMisspelled state so that ItemNotFound component can be rendered
    // accordingly
    setAsMisspelled(() => (foundMatch == false && suggestedWords.length < 0 ) ? false : true);
    ...
    return suggestedWords.length > 0 && foundMatch == false ? (
        ...
        <>
        <p className="ml-3 mt-3 font-bold" style={{fontFamily : "var( --font-anek)", color : "var(--color-primary)"}}>Did you mean...</p>
        {suggestedWords.filter((word) => word != "").map((word, index)=> (
            <ul key={index} onMouseDown={() => handleSelect(word)} className="text-left hover:bg-[#B6D7B0]" style={{fontFamily : "var( --font-anek)", color : "var(--color-primary)"}}>{word}</ul>
        ))}
        </>
