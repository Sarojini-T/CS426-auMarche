...
const MenuLink: React.FC<Props> = ({route, page}) => {
    ...
    return (
        <ul className="hover:bg-secondary w-full text-center">
            <a href={route} className={`mt-2 ml-2 mr-2 text-left text-white sm:${textSizes[0]} md:${textSizes[1]} lg:${textSizes[2]} xl:${textSizes[3]}`} style={{fontFamily : "var(--font-jomhuria)", textDecoration: 'none'}}>{page}</a>
        </ul>
    );
}
